"""
Adapter Albert LLM - Implémente LLMPort
...
class AlbertLLMAdapter(LLMPort):
    ...
    def __init__(self, api_key: str, model_name: str = DEFAULT_MODEL):
        """
        Initialise l'adapter Albert LLM.
        ...
        """
        if not api_key:
            raise LLMError("La clé API Albert est requise")
        ...
        try:
            ...
            logger.info(f"Albert LLM Adapter initialisé (modèle: {model_name})")
        except Exception as e:
            logger.error(f"Erreur initialisation Albert LLM: {e}")
            raise LLMError(f"Impossible d'initialiser Albert: {e}")
    ...
    def generate(
        ...
        try:
            ...
            if not content:
                raise LLMError("Aucune réponse générée par Albert")
            ...
            logger.error(f"Erreur génération Albert: {e}")
    ...
    def generate_with_history(
        ...
        try:
            ...
            logger.error(f"Erreur génération avec historique Albert: {e}")
            raise LLMError(f"Échec génération: {e}")
